...
| `/orchestrator/state` | `std_msgs/String` L | | | | P L | S | FSM state: `IDLE` / `NAVIGATING` / `JUNCTION` / `ROUNDABOUT` / `FALLBACK` / `EMERGENCY_STOP` / `DONE`. DB shows JUNCTION/FALLBACK/ROUNDABOUT badges + an EMERGENCY_STOP banner | ✔️ |
