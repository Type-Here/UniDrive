...
Owns all driving decisions:
  - Blends lane and map heading with a dynamic alpha
  - Handles junction rotation (disables lane, rotates to heading, re-enables)
  - Falls back to pure-pursuit when lane is lost

Blend formula (NAVIGATING state):
  angular.z = (1 - alpha) * lane_angular + alpha * map_angular
  linear.x  = lane_linear  (except full fallback: map_drive_speed)

Alpha is 0 on normal road (pure lane), ramps toward 1 when lane state
is HOLD/STOP, and is set to alpha_near_junction when the current target
node is a junction (map acts as a guardrail before the full maneuver).

FSM states
----------
  IDLE        No active goal (nav_info inactive)
  NAVIGATING  Blended lane+map control
  JUNCTION    In-place rotation toward next-waypoint heading, lane disabled
  FALLBACK    Pure-pursuit on planned path, lane disabled
...
class Orchestrator(object):
    ...
    _LANE_BAD = frozenset(("HOLD", "STOP", "DISABLED"))
    ...
    def __init__(self):
        ...
        self._alpha_junc     = float(rp("alpha_near_junction", 0.6))
        ...
        self._junc_align     = float(rp("junction_align_deg",  8.0))
        self._junc_spin      = float(rp("junction_spin_speed", 0.40))
        self._junc_creep     = float(rp("junction_creep_speed", 0.06))
        ...
        rospy.loginfo(
            "[orchestrator] ready. rate=%.0fHz  alpha_junc=%.2f  "
            "hold_ramp=%d  recovery=%d",
            self._rate_hz, self._alpha_junc,
            self._hold_ramp, self._recovery_ticks)
    ...
    def _step(self):
        ...
        # ======== JUNCTION ========
        if self._state == self.JUNCTION or (
                is_junction and next_id >= 0 and dist <= self._junc_radius):
            ...
            if abs(err) < math.radians(self._junc_align):
                # Aligned: creep forward, re-enable lane controller
                twist.linear.x = self._junc_creep
                self._cmd_pub.publish(twist)
                self._state = self.NAVIGATING
                self._set_lane_enabled(True)
                self._publish_orc_state(self.NAVIGATING)
                rospy.loginfo("[orchestrator] JUNCTION aligned -> NAVIGATING")
            else:
                twist.angular.z = clamp(
                    1.5 * err, -self._junc_spin, self._junc_spin)
                self._cmd_pub.publish(twist)
            return

        # ======== MAP FALLBACK ========
        if self._state == self.FALLBACK:
            if lane_state not in self._LANE_BAD:
                self._recv_count += 1
                if self._recv_count >= self._recovery_ticks:
                    ...
                    self._set_lane_enabled(True)
                    self._publish_orc_state(self.NAVIGATING)
                    rospy.loginfo("[orchestrator] FALLBACK -> NAVIGATING (lane recovered)")
                    # Fall through to NAVIGATING section below
                else:
                    # Hysteresis: lane visible but not confirmed yet, keep pursuing
        ...
        if next_id < 0:
            ...
            alpha = 0.0
            self._hold_count = 0
        elif lane_state in self._LANE_BAD:
            self._hold_count += 1
            if self._hold_count >= self._hold_ramp:
                # Lane lost long enough — enter full map fallback
                self._hold_count = 0
                self._recv_count = 0
                ...
                self._cmd_pub.publish(self._pursuit_twist(pose, path_ids))
                return
            # Ramp alpha smoothly toward 1 as hold_count grows
            alpha = self._hold_count / float(self._hold_ramp)
        else:
            self._hold_count = 0
            # Apply junction guardrail in the approach zone (before radius triggers rotation)
            alpha = self._alpha_junc if is_junction else 0.0
